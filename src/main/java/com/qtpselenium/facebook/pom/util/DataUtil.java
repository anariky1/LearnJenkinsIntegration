...
public static boolean isTestExecutable(Xls_Reader xls,String testCaseName){
	int rows =xls.getRowCount(FBConstants.TESTCASE_SHEET);
	for(int rNum=2;rNum<=rows;rNum++){
		String tcid=xls.getCellData(FBConstants.TESTCASE_SHEET, "TCID", rNum);
		if(tcid.equals(testCaseName)){
			String runMode=xls.getCellData(FBConstants.TESTCASE_SHEET, "RunMode", rNum);
			if(runMode.equals("Y")){
				return true;
			}else{
				return false;
